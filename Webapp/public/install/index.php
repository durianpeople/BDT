...
if($_POST["finale"] == "yes"){
	...
	/* Create config.php */
	$sample = file_get_contents("../../configs/root.sys.php.sample");
	$sample = str_replace("%DB_USER%",$_POST["dbuser"],$sample);
	...
	$sample = str_replace("%SMTP_HOST%","",$sample);
	$sample = str_replace("%SMTP_USER%","",$sample);
	$sample = str_replace("%SMTP_PASS%","",$sample);
	$sample = str_replace("%SMTP_ENCR%","none",$sample);
	$sample = str_replace("%SMTP_PORT%","25",$sample);
	$sample = str_replace("%SMTP_AUTH%","false",$sample);
	$sample = str_replace("%ERR_CODE%",0,$sample);
	$sample = str_replace("%USE_MDM%","false",$sample);
	$sample = str_replace("%LANG%","loc",$sample);
	$sample = str_replace("%SITENAME%", $_POST["sitename"],$sample);
	$sample = str_replace("%TIMEZONE%",$_POST["timezone"],$sample);
	$sample = str_replace("%COPYRIGHT%","",$sample);
...
<html>
	<head>
		...
		<script>
		var installing_now = false;
		$(window).on("load",function(){
			$("button.tabSwitch").on("click",function(){
				if(!$(this).hasClass("back")){
					let y=$(this).closest("div.tab").find("input[required]").filter(function() {return this.value=="";});
					if(y.length > 0){
				...
				document.location.hash = $(this).attr("next-tab");
			});
			if(document.location.hash == "#welcome") $(window).trigger("hashchange");
			document.location.hash = "welcome";
			$("input").on("keypress",function(e){				
				if(e.keyCode==13){
					e.preventDefault();
					$(this).closest("div.tab").find("button.tabSwitch").last().trigger("click");
				}
			});
			$("#install_frm").on("submit", function(e){
				e.preventDefault();
			});
			$("button.submit").on("click",function(){
				$(".has-error").removeClass("has-error");
				if($("#step1 input[required]").filter(function() {return this.value=="";}).length > 0){
					document.location.hash = "step1";
					$("#step1 input[required]").filter(function() {return this.value=="";}).addClass("has-error");
					return;
				}
				if($("#step2 input[required]").filter(function() {return this.value=="";}).length > 0){
					document.location.hash = "step2";
					$("#step2 input[required]").filter(function() {return this.value=="";}).addClass("has-error");
					return;
				}
				if($(".phpcheck").filter(function(){
					return($(this).html().trim() == "No");
				}).length > 0){
					alert("Cannot install PuzzleOS! Please complete all PHP requirement checklist!");
					$("button.submit").prop("disabled",true).html("Please refresh this page!");
					return;					
				}
				let form = new FormData($("#install_frm").get(0));
				form.append("finale","yes");
				$.ajax({
					...
					beforeSend: function(){
						...
						$("button.submit").html("Installing...");
						$(".info").hide();
					},
					success: function(d){
						$("button").prop("disabled", false);
						$("button.submit").html("Install");
						switch(d){
						case "2":
							$(".info").html("Cannot send data to server!").show();
							break;
						case "1":
		...
		$(window).on("hashchange",function(){
			if(installing_now) return;
			if($(document.location.hash).length < 1) return;
			let x = $(this);
			$("div.tab").animate({"margin-left":-20,"opacity":0},200,function(){
				$(this).hide().css("margin-left",0).css("opacity",1);
	...
	<body>
		<div id="wrap">
			...
			<div id="step1" class="tab">
				<div class="sub-title">Step One</div><br>
				<div class="row">
				...
				<div class="row">
					...
					<div class="col-lg-9 row" style="margin:0px;">
						<?php
							...
							foreach(DateTimeZone::listIdentifiers(DateTimeZone::ALL) as $d){
			...
			<div id="step2" class="tab">
				...
				<button next-tab="step3" type="button" class="tabSwitch btn btn-primary btn-big">Next</button>
			</div>
			<div id="step3" class="tab">
				<div class="sub-title">Finale</div><br>
				<?php if(0):?>
				<div class="row">
					<label class="col-lg-9 control-label">Do you want to enable PuzzleOS SDK for development?</label>
					<div class="col-lg-3 row" style="margin:0px;">
						<select class="form-control" name="sdk-enable">
